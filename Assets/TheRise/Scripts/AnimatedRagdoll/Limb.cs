...
public abstract class Limb : MonoBehaviour
{
    ...
    protected Collider limbCollider;
    ...
    public virtual void ProjectileHit(Vector3 force)
    {
        limbRb.AddForce(force ,ForceMode.VelocityChange);
    }
    ...
    protected virtual void JointBreak(float breakForce)
    {
        //Debug.Log($"Joint breaked on {this.name}");

        mySkeleton.JointBreak(limbProfile.pBreakForce);
        // this.transform.parent = null;
        if (this.transform.GetChild(0)) 
        {
            var limb = this.transform.GetChild(0).GetComponentInChildren<Limb>();
            if(limb)
                limb.JointBreak(breakForce);
        }
    ...
    protected virtual void SetLimb()
    {
        //if (FollowRate != limbProfile.pFollowRate)
        {
            ...
            FollowTorque = mySkeleton.PTorque * mySkeleton.animationRate * limbProfile.pAnimationRate;
            ...
            BreakForce = limbProfile.pBreakForce * mySkeleton.BreakForce;
    ...
    protected virtual void AddFollowForce(Transform followedLimb)
    {
        ...
        limbRb.AddForce(forceSignal, ForceMode.Force);
    }
    ...
    protected virtual void AddFollowTorque(Transform followedLimb)
    {
        ...
        torqueError = GeneralUtils.FixEuler(torqueAngle) * torqueAxis;
        ...
        if(torqueSignal != new Vector3())
            limbRb.AddTorque(torqueSignal, ForceMode.Force); // Add torque to the limbs
    ...
    void SetJointTorque()
    {
        ...
        limbJoint.breakForce = BreakForce;
    }
