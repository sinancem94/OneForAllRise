...
public class Skeleton : MonoBehaviour
{
	...
	[Range(10f, 10000f)] public float maxJointSpring;
	public RangeAttribute jointSpringRange = new RangeAttribute(10f, 150f);

	[Range(0f, 1000f)] public float PTorque = 0.5f; // For all limbs Torque strength
	[Range(0f, 1000f)] public float DTorque = 0.5f; // For all limbs Torque strength
	[Range(0f, 1000f)] public float PForce = 5f; // For all limbs Force strength
	[Range(0f, 1000f)] public float DForce = 0.05f;

	[SerializeField] bool hideAnimated = true;
	[SerializeField] public bool useGravity = false; // Ragdoll is affected by Unitys gravity

	[Range(0f, 1000f)] public float angularDrag = 100f; // Rigidbodies angular drag. Unitys parameter
	[Range(0f, 1000f)] public float drag = 10f; // Rigidbodies drag. Unitys parameter

	[SerializeField] public PhysicMaterial physicMat;
	...
	[SerializeField] public bool torque = true; // Use World torque to controll the ragdoll (if true)
	[SerializeField] public bool force = true; // Use force to controll the ragdoll
	[SerializeField] public bool follow = true; // Use force to controll the ragdoll
	...
	private readonly object breakLock = new object();
	...
	private void Awake()
	{
		...
		{
			if (ragdollTransform.GetComponent<Rigidbody>() && ragdollTransform != this.transform)
			{
				if (!rootBone)
	...
	public void JointBreak(float breakFactor)
	{
		lock (breakLock)
		{
			//animationRate *= 0.75f;
			if (onJointBreak != null)
				onJointBreak(breakFactor);
