...
namespace AwesomeUtils
{
    ...
    public class GeneralUtils
    {
        ...
        public static float FixEuler(float angle) // For the angle in angleAxis, to make the error a scalar
        {
            if (angle > 180f)
                return angle - 360f;
            else
    ...
    public class MathfUtils
    {
        ...
        //Basically, generate a new vector that blends between two others based on a random value.
        public static Vector2 RandomV2(Vector2 min, Vector2 max){
            return Vector2.Lerp(min,max, UnityEngine.Random.value).normalized;
        }
    }
    #endregion
    ...
    {
        public static Vector3 RotateInputDirection(Vector3 curRot,float vertical,float horizontal)
        {
            if (vertical == 0f && horizontal == 0f) { // this statement allows it to recenter once the inputs are at zero 
                Vector3 homeRot;
                ...
                return  Vector3.Slerp (curRot, homeRot, Time.deltaTime*2);
            } else {
                return  new Vector3 (0f, Mathf.Atan2 (horizontal, vertical) * 180 / Mathf.PI, 0f); // this does the actual rotaion according to inputs
            }
        }
        
        public static bool IsHitDirection(Transform transform, Vector3 direction,float radius,float rayLength , int sensitivity, int rayLayer)
        {
            ...
            {
                float angle = i * Mathf.PI * 2f / sensitivity;
                Vector3 rayStartPos = center + new Vector3(Mathf.Cos(angle)*radius, 0f, Mathf.Sin(angle)*radius);
                Ray downRay = new Ray(rayStartPos,direction);
#if UNITY_EDITOR
                Debug.DrawRay(rayStartPos, direction * (rayLength), Color.green,0.1f);
#endif
                if (Physics.Raycast(downRay, out hit, rayLength, rayLayer))
                {
                    hitCounter++;
                }
            }
